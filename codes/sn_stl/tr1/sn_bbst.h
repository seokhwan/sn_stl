		...
		*/
		...
		public:
			...
			public:
				...
				iterator operator++()
				{
					if ((*this) != p_tree->m_iter_end)
					{
						if ((*this) == p_tree->m_iter_max)
						{
							p_cur = p_tree->m_iter_end.p_cur;
						}
						else
						{
							...
							{
								if (this->p_cur->p_parent->elem.first > this->p_cur->elem.first)
								{
									p_cur = p_cur->p_parent;
								}
								else
								{
									node* p_parent = p_cur->p_parent->p_parent;
									while (this->p_cur->elem.first > p_parent->elem.first)
									{
			...
			{
				return const_iterator(_find_min(m_p_root), this);
			}
			...
			{
				return iterator(_find_min(m_p_root), this);
			}
